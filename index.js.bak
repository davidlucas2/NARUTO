...
const {leveling} = JSON.parse(fs.readFileSync('./src/leveling/leveling.json
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao melhor grupo*${mdata.subject}*\nPor favor não seja um ghost❤️`
				...
				teks = `VAI EMBORA  @${num.split('@')[0]} LOGO FDP👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Thomas/n*WPP:* wa.me/+556195993572\n*INSTA:* to sem aff kk\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} agora é admin, parabéns fdp`, mentioned, true)
